...
failure.  We could not think of a way to parse multiple key-value pairs using iterators.  The intended plan was ```node iterator.second[label=”iterator.first”]  ->   next node``` and so on.  
